% This program implements Wirtinger Calculus based Levenberg-Marquardt
% algorithm for complex-valued neural network (a three layer feed-forward)
% 
...
MAX_EPOCH = 100;    % Maximum number of epoch
...
for epoch = 1:MAX_EPOCH
    ...
    while (mu<=MU_MAX)
        % Gauss_Newton Method
        m = size(J,2);
        H11 = J'*J + conj(JC'*JC) + mu*eye(m);
        H12 = J'*JC + conj(JC'*J);
        H21 = conj(H12);
        H22 = conj(H11) + mu*eye(m);
        M = H12*pinv(H22);
        b = J'*e + conj(JC'*e);
        deltaZ = (H11 - M*H21)\(b - M*conj(b));
